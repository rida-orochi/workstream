...
int get_sentence()
{
    ...
    // crearting array that stores 20 sentences
    char para[20][200];
    ...
    do
    {
        // display menu to the user to choose from it
        printf("\n\t-main menu:");
...
// loop too display each sentence in a new line
    for (int i = 0; i <= 20; i++)
    {
// condition to print sentence if sentence is not empty:
        if (strlen(Pointer_1[0] + (i * 200)) != 1)
        {
            printf("\n%s", Pointer_1[0] + (i * 200));
        }
        // when to stop and break the loop:
        if (strlen(Pointer_1[0] + (i * 200)) == 1) // an empty sentence originally contains 1 element (/0)
        {
            break;
        }
    }
...
int select_sentence(char *Pointer_2[])
{
//getting the number of the chosen sentence from the user
...
//getting user choice for the submenu
        int user_choice;
        scanf("%d", &user_choice);
        // Display sentence
        if (user_choice == 1)
        {
            sentence_number -= 1;
            printf("\n-the sentences you have chosen : ");
            printf("\n%s", Pointer_2[0] + (sentence_number * 200));
        }
        // modify sentence
        else if (user_choice == 2)
        {
//creating array that will store the new replacement 
            char sentence_r[200];
            getchar();
            int reference = (sentence_number - 1) * 200;
            sentence_number -= 1;
//telling  the user the sentence that has been chosen 
            printf("\n-the sentence you are now working on is: ");
            printf("\n%s", Pointer_2[0] + (sentence_number * 200));
//getting the new sentence
